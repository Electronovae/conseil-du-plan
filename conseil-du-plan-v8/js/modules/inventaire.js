 ...
 * Gère : coffre de guilde, inventaire personnel, richesse, marché, marchands, investissements
 */

// ================================================================
// ÉTAT DU MODULE
// ================================================================
let invTab = 'guild';           // 'guild' | 'member'
let invMemberId = DB.members[0]?.id || null;
let invSearch = '';
let invViewMode = 'list';       // 'list' | 'grid'
let memberInvViewMode = 'list'; // 'list' | 'grid'
let invGuildTab = 'items';      // 'items' | 'wealth' | 'market' | 'merchants' | 'invest' | 'chests'
...
function renderInventory(){
  ...
  <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;align-items:center">
    ...
    <!-- Barre de recherche + bouton ajout (visible sur les onglets avec liste d'objets) -->
    ...
    <!-- Bouton "Mettre en vente" (visible uniquement sur l'onglet Marché) -->
...
/** Change l'onglet principal (guild / member) et réinitialise le sous-onglet */
...
function renderInventoryList(){
  // Source des items selon l'onglet actif
  ...
  // Badge de propriétaire ou de coffre affecté (mode guilde uniquement)
  const ownerTag = item => {
    const tags = [];
...
function renderWealthPanel(){
  ...
  // Valeur totale en PO des dépôts membres (affichage dans le titre de section)
  const memberBankPO = DB.members.reduce((s, m) => {
    const b = m.bank || {};
    return s + Math.round((b.po||0) + (b.pp||0)*10 + (b.pa||0)*0.1 + (b.pc||0)*0.01);
  }, 0);
  ...
  <div style="display:flex;flex-direction:column;gap:20px;max-width:700px">
    ...
    <div class="card">
      ...
      <!-- Grille des pièces (PP / PO / PA / PC) -->
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px">
        ${coinRows}
      </div>

      <!-- Section lingots d'or + valeur totale -->
    ...
    <!-- NOTE : les fonds communs de la banque (DB.guildBank) sont supprimés de
         ...
         Les dépôts individuels des membres (m.bank) sont conservés. -->
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        <div style="font-size:11px;color:var(--indigo);font-weight:700;letter-spacing:.1em;text-transform:uppercase">🎒 Dépôts des Aventuriers</div>
        ${memberBankPO > 0 ? `<span style="font-size:12px;color:var(--gold);font-weight:700">${memberBankPO.toLocaleString()} PO au total</span>` : ''}
      </div>

      <!-- Liste des dépôts par membre (membres avec dépôt > 0 PO seulement) -->
      ${DB.members.length ? `
      <div style="display:flex;flex-direction:column;gap:5px">
        ${DB.members.map(m => {
          const b   = m.bank || {};
          const bPO = Math.round((b.po||0) + (b.pp||0)*10 + (b.pa||0)*0.1 + (b.pc||0)*0.01);
...
function renderGuildChestsPanel(){
  ...
  <div style="display:flex;flex-direction:column;gap:16px;max-width:700px">
    ...
    <!-- Carte par coffre avec ses objets -->
    ${chests.map((ch, ci) => {
...
/** Sauvegarde un coffre (création ou édition) */
...
/** Supprime un coffre et replace ses objets dans le coffre commun */
function deleteGuildChest(idx){
  ...
  // Réaffecte les objets au coffre commun (supprime le chestId)
...
function renderMarketPanel(){
  ...
  <div style="display:flex;flex-direction:column;gap:10px">
    ${market.length===0
      ? `<div style="text-align:center;color:var(--dim);padding:40px;font-style:italic">Aucun objet en vente. Mettez des objets du Coffre en vente avec le bouton "+ Mettre en vente".</div>`
      : market.map(item => {
          const rc = RARITY[item.rarity]?.col || '#9ca3af';
...
/** Ouvre la modale pour choisir un objet du coffre à mettre en vente */
...
/** Ajoute un objet du coffre au marché (crée une entrée miroir dans DB.market) */
function addToMarket(){
  ...
  // Migrations V5 / V6 appliquées à tous les membres au passage
...
/** Exécute l'achat : débite le membre, retire du coffre, transfère dans l'inventaire perso */
function executeBuy(marketItemId){
  ...
  // Ajout dans l'inventaire du membre
  ...
  // Crédit de la trésorerie de la guilde
...
function openInvModal(editId, preChestId){
  ...
  openModal(`<div class="modal" onclick="event.stopPropagation()">
    ...
    <div class="modal-body">
      ...
      <!-- Panneau de recherche dans la base d'items (togglable via toggleItemDBPanel) -->
      ...
      <!-- Bonus de capacité (visible uniquement si catégorie = Sac) -->
      ...
      <!-- Sélecteur de coffre (mode guilde uniquement, si des coffres existent) -->
...
/** Sauvegarde un objet (coffre ou inventaire perso) */
function saveInvItem(editId){
  const item = window._editItem;
  if (!item || !item.name.trim()) return;

  // Remplace l'icône par l'upload si présent
...
function openInvTransfer(itemId){
  ...
  // Options de destination selon la source
...
/** Effectue le transfert d'un objet d'une source vers une destination */
function doTransfer(itemId){
  const qty  = +(window._trQty || 1);
  const dest = String(window._trDest || '');

  // Ajoute qty unités de src dans le coffre de guilde, avec ownerId optionnel
  ...
  if (invTab==='guild') {
    ...
    if (dest.startsWith('inv_')) {
      // Vers l'inventaire perso d'un membre
...
/** Retourne (ou génère) le seed aléatoire d'un marchand */
...
/** Force le renouvellement du stock d'un marchand spécifique */
...
/**
 * Génère le stock aléatoire d'un marchand à partir de son seed.
 * Utilise la liste DB.merchantItems[key] si définie, sinon ITEM_DATABASE filtré par merchant.
 */
function generateMerchantStock(merchantKey){
  const m = getMerchantsAll()[merchantKey];
  if (!m) return [];

  // Sélectionne le pool d'items : custom ou défaut
  let pool;
  ...
  // Mélange déterministe basé sur le seed
  ...
  // Applique un modificateur de prix aléatoire (×0.5 à ×4)
...
/** Renouvelle le stock de TOUS les marchands */
...
/** Sauvegarde un marchand dans DB.merchants (crée ou met à jour) */
...
/** Retourne DB.merchants si renseigné, sinon les marchands par défaut (MERCHANTS) */
...
/** Coche / décoche tous les items d'une catégorie dans la modale marchand */
...
/** Réinitialise les items d'un marchand aux valeurs par défaut (supprime la custom list) */
...
/** Enregistre la liste d'items custom d'un marchand */
...
/** Affiche le panneau des marchands avec leurs stocks générés */
...
/** Ouvre la modale d'achat chez un marchand */
...
/** Exécute l'achat chez un marchand : débite et ajoute l'item à la guilde ou au membre */
function execMerchantBuy(itemName, price){
  ...
  if (buyerVal==='guild') {
    // Achat pour le coffre de guilde
...
/** Table des résultats d'investissement (5d6 → pourcentage de gain/perte) */
const INVEST_TABLE = [
...
/** Retourne la ligne de résultat correspondant au jet de dés */
function getInvestResult(roll){
  return INVEST_TABLE.find(r => roll>=r.min && roll<=r.max) || INVEST_TABLE[3];
}

/** Lance 5d6, applique le gain/perte sur le capital et enregistre dans l'historique */
function rollInvestment(){
  ...
  inv.capital += gain;
  if (inv.capital < 0) inv.capital = 0;
  inv.history.push({
...
/** Transfère un montant de la trésorerie vers le capital investi */
...
/** Retire un montant du capital investi vers la trésorerie */
...
/** Efface l'historique des placements (conserve le capital) */
...
/** Affiche le panneau investissement avec courbe et historique */
function renderInvestPanel(){
  ...
  if (pts.length < 2) pts = [inv.capital, inv.capital];
  ...
  el.innerHTML = `<div style="max-width:750px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px">
      <div class="card" style="background:linear-gradient(135deg,#1a1207,#2d1f0e);border:1px solid #8b691444">
        ...
        <div style="display:flex;gap:6px">
          <input id="invest-amount" type="number" min="1" placeholder="PO" style="flex:1;background:#0d0a05;border:1px solid #8b691444;border-radius:6px;color:#fbbf24;padding:8px;font-family:'Cinzel',serif;font-size:14px">
      ...
      <div class="card" style="background:linear-gradient(135deg,#1a1207,#2d1f0e);border:1px solid #8b691444">
        ...
        <div style="display:flex;gap:6px">
          <input id="withdraw-amount" type="number" min="1" max="${inv.capital}" placeholder="PO" style="flex:1;background:#0d0a05;border:1px solid #8b691444;border-radius:6px;color:#fbbf24;padding:8px;font-family:'Cinzel',serif;font-size:14px">
    ...
    ${history.length > 0 ? `<div class="card" style="background:linear-gradient(135deg,#1a1207,#2d1f0e);border:1px solid #8b691444;margin-bottom:20px">
      ...
      <svg viewBox="0 0 ${cW} ${cH}" style="width:100%;height:auto">
        <defs><linearGradient id="investGrad" x1="0" y1="0" x2="0" y2="1">
    ...
    ${history.length > 0 ? `<div class="card" style="background:linear-gradient(135deg,#1a1207,#2d1f0e);border:1px solid #8b691444">
      ...
      <button class="btn btn-danger btn-xs" onclick="resetInvestments()" style="float:right" title="Réinitialiser">🗑️ Reset</button>
      <div style="display:flex;flex-direction:column;gap:4px">
